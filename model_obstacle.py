...
class DQN:
    ...
    def __init__(self, action_space, state_space):
        ...
        if False:
            self.model = keras.models.load_model('model_attack9')
        else:
...
def train_dqn(episode):
    ...
    for e in range(episode):
        ...
        while itera < max_steps and hit < max_hit:
            ...
            score += reward
            if goal > record or hit > 99: 
                record = goal
                agent.model.save('model_obstacle')
    ...
    agent.model.save('model_obstacle.1')

    
    return loss, agent,nb_hit,nb_goal


if __name__ == '__main__':

    ep = 177 #number of games played for training
    loss, model,nb_hit,nb_goal = train_dqn(ep)
